    ...
    public class Controller : BaseOnlineController
    {
        ...
        async public Task<ActionResult> Play(string hash, string title)
        {
            ...
            if (result.Streams != null && result.Streams.Count > 0)
            {
                ...
                return UpdateService.Validate(Content(VideoTpl.ToJson("play", streamUrl, videoTitle, streamquality: streamQuality), "application/json; charset=utf-8"));
            }

            string defaultUrl = BuildStreamUrl(init, result.Stream);
            return UpdateService.Validate(Content(VideoTpl.ToJson("play", defaultUrl, videoTitle), "application/json; charset=utf-8"));
        }
