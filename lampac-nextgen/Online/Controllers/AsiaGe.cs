...
using Online.Models.AsiaGe;

namespace Online.Controllers
{
    public class AsiaGe : BaseOnlineController
    {
        public AsiaGe() : base(ModInit.siteConf.AsiaGe) { }
        ...
        async public Task<ActionResult> Index(long id, string title, int year, int serial = 0, string href = null, bool similar = false)
        {
            ...
            if (string.IsNullOrEmpty(href))
            {
                ...
                rhubSearchFallback:
                var search = await InvokeCacheResult<EmbedModel>($"asiage:search:{searchTitle}:{year}", 40, async e =>
                {
