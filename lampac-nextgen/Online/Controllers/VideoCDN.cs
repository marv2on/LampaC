...
namespace Online.Controllers
{
    ...
    {
        public VideoCDN() : base(ModInit.siteConf.VideoCDN)
        {
        ...
        async public Task<ActionResult> Index(long content_id, string content_type, string imdb_id, long kinopoisk_id, string title, string original_title, string t, int clarification, bool similar = false, int s = -1, int serial = -1, bool rjson = false, bool checksearch = false)
        {
            ...
            if (content_id == 0)
            {
                ...
                if (search.similar?.data != null)
                    return ContentTpl(search.similar);
        ...
        rhubFallback:
            var hlsCache = await InvokeCacheResult<string>(memkey, ttl, async e =>
            {
            ...
            if (IsRhubFallback(hlsCache))
                goto rhubFallback;

            if (!hlsCache.IsSuccess)
                return OnError(hlsCache.ErrorMsg, gbcache: false);

            string hls = hlsCache.Value;
            ...
            {
                Serilog.Log.Error(ex, "CatchId={CatchId}", "id_pdk9noed");
            }
            #endregion
