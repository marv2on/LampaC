...
namespace Online.Controllers
{
    ...
    {
        public AnimeGo() : base(ModInit.animeConf.AnimeGo) { }
        ...
        async public Task<ActionResult> Index(string title, int year, int pid, int s, string t, bool similar = false)
        {
            ...
            {
                #region Поиск
                var cache = await InvokeCacheResult<List<(string title, string year, string pid, string s, string img)>>($"animego:search:{title}", 40, async e =>
                {
