...
namespace Online.Controllers
{
    ...
    {
        public MoonAnime() : base(ModInit.animeConf.MoonAnime) { }
        ...
        async public Task<ActionResult> Index(string imdb_id, string title, string original_title, long animeid, string t, int s = -1, bool rjson = false, bool similar = false)
        {
            ...
            #region Поиск
            rhubFallback:
                var cache = await InvokeCacheResult<List<(string title, string year, long id, string poster)>>($"moonanime:search:{imdb_id}:{title}:{original_title}", 40, async e =>
                {
                    async Task<JObject> goSearch(string arg)
                    {
                        ...
                        var search = await httpHydra.Get<JObject>($"{init.host}/api/2.0/titles?api_key={init.token}&limit=20" + arg, safety: true);
            ...
            #region Серии
            rhubFallback:
                var cache = await InvokeCacheResult<JArray>($"moonanime:playlist:{animeid}", 30, async e =>
                {
                    var root = await httpHydra.Get<JArray>($"{init.host}/api/2.0/title/{animeid}/videos?api_key={init.token}", safety: true);
