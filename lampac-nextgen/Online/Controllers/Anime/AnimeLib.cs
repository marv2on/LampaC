...
namespace Online.Controllers
{
    ...
    {
        static readonly Serilog.ILogger Log = Serilog.Log.ForContext<AnimeLib>();

        public AnimeLib() : base(ModInit.animeConf.AnimeLib) { }
        ...
        async public Task<ActionResult> Index(string title, string original_title, int year, string uri, string t, bool rjson = false, bool similar = false)
        {
            ...
            if (string.IsNullOrWhiteSpace(uri))
            {
                ...
                rhubFallback:
                var search = await InvokeCacheResult<List<(string title, string year, string uri, bool coincidence, string cover)>>($"animelib:search:{title}:{original_title}", 40, async e =>
                {
            ...
            #region Серии
            rhubFallback:
                var cache = await InvokeCacheResult<Episode[]>($"animelib:playlist:{uri}", 30, async e =>
                {
                    ...
                    var episodes = root["data"].ToObject<Episode[]>();
                ...
                if (!hybridCache.TryGetValue(voice_memkey, out Player[] players))
                {
                    ...
                    if (root["data"] is JObject data && data.ContainsKey("players"))
                        players = data["players"].ToObject<Player[]>();
        ...
        rhubFallback:
            var cache = await InvokeCacheResult<Player[]>($"animelib:video:{id}", 30, async e =>
            {
                ...
                if (root["data"] is JObject data && data.ContainsKey("players"))
                    return e.Success(data["players"].ToObject<Player[]>());

                return e.Fail("players");
            });
