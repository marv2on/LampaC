...
namespace Online.Controllers
{
    public class AniLiberty : BaseOnlineController
    {
        ...
        public AniLiberty() : base(ModInit.animeConf.AniLiberty)
        {
        ...
        async public Task<ActionResult> Index(string title, int year, string releases, bool rjson = false, bool similar = false, string source = null, string id = null)
        {
            ...
            if (string.IsNullOrEmpty(releases))
            {
                ...
                {
                    var search = await httpHydra.Get<JArray>($"{init.host}/api/v1/app/search/releases?query={HttpUtility.UrlEncode(title)}");
            ...
            rhubFallback:
                ...
                {
                    var root = await httpHydra.Get<JObject>($"{init.host}/api/v1/anime/releases/{releases}");
