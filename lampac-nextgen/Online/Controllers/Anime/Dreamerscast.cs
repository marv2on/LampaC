...
using Online.Models.Dreamerscast;
...
namespace Online.Controllers
{
    ...
    {
        public Dreamerscast() : base(ModInit.animeConf.Dreamerscast) { }
        ...
        async public Task<ActionResult> Index(string title, int year, string uri, int s = 1, bool rjson = false, bool similar = false)
        {
            ...
            if (string.IsNullOrWhiteSpace(uri))
            {
                ...
                rhubFallback:
                var cache = await InvokeCacheResult<List<SearchItem>>($"dreamerscast:search:{title}", 30, textJson: true, onget: async e =>
                {
