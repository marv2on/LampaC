...
namespace Online.Controllers
{
    ...
    {
        public AniMedia() : base(ModInit.animeConf.AniMedia) { }
        ...
        async public Task<ActionResult> Index(string title, string news, bool rjson = false, bool similar = false)
        {
            ...
            if (string.IsNullOrEmpty(news))
            {
                ...
                #region Поиск
                var cache = await InvokeCacheResult<List<(string title, string url, string img)>>($"animedia:search:{title}:{similar}", 40, async e =>
                {
                    bool reqOk = false;
            ...
            {
                #region Серии
                var cache = await InvokeCacheResult<List<(int episode, string s, string vod)>>($"animedia:{news}", 30, async e =>
                {
        ...
        async public Task<ActionResult> Video(string vod)
        {
            ...
            var cache = await InvokeCacheResult<string>($"animedia:{vod}", 180, async e =>
            {
                string hls = null;
