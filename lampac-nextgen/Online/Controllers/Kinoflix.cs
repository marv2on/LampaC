...
using Online.Models.Kinoflix;
using Shared.Services.RxEnumerate;

namespace Online.Controllers
{
    public class Kinoflix : BaseOnlineController
    {
        public Kinoflix() : base(ModInit.siteConf.Kinoflix) { }
        ...
        async public Task<ActionResult> Index(string title, string original_title, int clarification, int year, string href, string t = null, int s = -1)
        {
            ...
            {
                var search = await InvokeCacheResult<SearchModel>($"kinoflix:search:{searchTitle}", 60, async e =>
                {
            ...
            var cache = await InvokeCacheResult<CardModel>(ipkey($"kinoflix:iframe:{href}"), 20, textJson: true, onget: async e =>
            {
                ...
                await httpHydra.GetSpan(iframeUri, spanAction: html =>
                {
                    if (html.Contains("\"folder\":", StringComparison.Ordinal))
                    {
                        ...
                        if (json != null)
                            card.seasons = JObject.Parse(json)["file"].ToObject<List<Season>>();
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (s == -1)
                    {
                        ...
                        foreach (var season in seasons)
                        {
                            ...
                            string link = $"{host}/lite/kinoflix?title={HttpUtility.UrlEncode(title)}&original_title={HttpUtility.UrlEncode(original_title)}&s={seasonNum}";
                    ...
                    else
                    {
                        ...
                        string defaultargs = $"&href={HttpUtility.UrlEncode(href)}&title={HttpUtility.UrlEncode(title)}&original_title={HttpUtility.UrlEncode(original_title)}&year={year}";
        ...
        async Task<SearchModel> Search(string query, string title, string original_title, int year)
        {
            ...
            await httpHydra.GetSpan($"{init.host}/filter-movies?type=search&search={HttpUtility.UrlEncode(query)}", html =>
            {
                ...
                foreach (ReadOnlySpan<char> row in HtmlSpan.Nodes(html, "div", "class", "filter_cols", HtmlSpanTargetType.Exact))
                {
                    ...
                    similar.Append(orig_name != null ? $"{name} / {orig_name}" : name, _year, string.Empty, href, PosterApi.Size(img));
