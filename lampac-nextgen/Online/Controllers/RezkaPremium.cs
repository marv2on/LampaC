...
namespace Online.Controllers
{
    public class RezkaPremium : BaseOnlineController<RezkaSettings>
    {
        ...
        string machineId = null, machineIdType;

        sealed record uidModel(string uid, string typeuid);
        static readonly ConcurrentDictionary<string, uidModel> uids = new();
        ...
        public RezkaPremium() : base(ModInit.siteConf.RezkaPrem)
        {
            ...
            if (machineId == null)
            {
                try
                {
                    machineId = Regex.Replace(System.IO.File.ReadAllText("/etc/machine-id"), "[^a-zA-Z0-9\\-]+", "").Trim();
                    machineIdType = "machine-id";
                }
                catch
                {
                    machineId = Guid.NewGuid().ToString();
                    machineIdType = "random-id";
                }
            }
            #endregion
            ...
            {
                init.host = new RezkaSettings(null, "https://lamp.ac").host;
        ...
        {
            if (!uids.TryGetValue(cookie, out uidModel uidType))
            {
                if (requestInfo.AesGcmKey != null)
                    uidType = new(CrypTo.md5(requestInfo.AesGcmKey), "kit");
                else
                    uidType = new(machineId, machineIdType);
            }

            var headers = httpHeaders(init, HeadersModel.Init(
               ("X-Lampac-App", "1"),
               ("X-Lampac-Version", "NextGen"),
               ("X-Lampac-Device-Id", $"{(CoreInit.Win32NT ? "win32" : "linux")}/{uidType.typeuid}:{uidType.uid}"),
        ...
        async public Task<ActionResult> Index(string title, string original_title, int clarification, int year, int s = -1, string href = null, bool rjson = false, int serial = -1, bool similar = false, string source = null, string id = null)
        {
            ...
            {
                var search = await InvokeCacheResult<SearchModel>($"rhsprem:search:{title}:{original_title}:{clarification}:{year}", 40, textJson: true, onget: async e =>
                {
        ...
        async public Task<ActionResult> Serial(string title, string original_title, string href, long id, int t, int s = -1, bool rjson = false, bool similar = false)
        {
            ...
            return ContentTpl(oninvk.Serial(cache_root.Value, cache_content.Value, accsArgs(string.Empty), title, original_title, href, id, t, s, true, rjson));
        }
        #endregion
        ...
        #region getCookie
        static readonly ConcurrentDictionary<string, string> authCookie = new();
        ...
        async ValueTask<(string cookie, string log)> getCookie(RezkaSettings init, int timeoutError = 15)
        {
            ...
            {
                Serilog.Log.Error(ex, "CatchId={CatchId}", "id_22383ea3"); loglines += $"\n\nHDRezka Exception: {ex}";
            }
