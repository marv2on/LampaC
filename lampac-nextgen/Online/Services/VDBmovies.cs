...
namespace Online.Services
{
    public struct VDBmoviesInvoke
    {
        ...
        public string DecodeEval(string file)
        {
            ...
            try
            {
                using (var nbuf = new BufferBytePool(Encoding.UTF8.GetMaxByteCount(file.Length)))
                {
                    if (Convert.TryFromBase64String(file, nbuf.Span, out int bytesWritten))
                        return Encoding.UTF8.GetString(nbuf.Span.Slice(0, bytesWritten));
                }
            }
            catch (System.Exception ex)
            {
                Serilog.Log.Error(ex, "{Class} {CatchId}", "VDBmovies", "id_bxozphtp");
            }
        ...
        public EmbedModel Embed(string json, string forbidden_quality, string default_quality)
        {
            ...
            try
            {
                ...
                {
                    var serial = JsonSerializer.Deserialize<Models.CDNmovies.Voice[]>(json, new JsonSerializerOptions
                    {
                ...
                {
                    var movies = JsonSerializer.Deserialize<Episode[]>(json, new JsonSerializerOptions
                    {
                        AllowTrailingCommas = true
                    });
        ...
        public ITplResult Tpl(EmbedModel root, string orid, string imdb_id, long kinopoisk_id, string title, string original_title, string t, int s, int sid, VastConf vast = null, bool rjson = false)
        {
            ...
            if (root.movies != null)
            {
                ...
                foreach (var m in root.movies)
                {
                    ...
                    if (first != null)
                        mtpl.Append(m.title, first.link, subtitles: subtitles, streamquality: streamquality, vast: vast);
                }
                ...
                else
                {
                    ...
                    foreach (var episode in root.serial[sid].folder)
                    {
                        ...
                        foreach (var voice in episode.folder)
                        {
                            ...
                            var first = streamquality.Firts();
                            if (first != null)
                                etpl.Append($"{ename} cерия", title ?? original_title, sArhc, ename, first.link, streamquality: streamquality, vast: vast);
                        }
                    }
        ...
        static readonly string[] _qualitys = ["1080", "720", "480", "360", "240"];
        static Regex targetRegex = new Regex("\\[(?<quality>[^\\]]+)\\](?<m3u8>https?://[^\\[\\|,\n\r\t ]+\\.m3u8)", RegexOptions.Compiled);
        static Regex linkRegex = new Regex("\\[([^\\]]+)\\](https?://[^\\[\\|,\n\r\t ]+\\.m3u8)", RegexOptions.Compiled);

        StreamQualityTpl getStreamQualityTpl(string file)
        {
            var streamquality = new StreamQualityTpl();

            var target = targetRegex.Match(file).Groups;
            string targetQuality = target["quality"].Value.Replace("p", "");

            foreach (string q in _qualitys)
            {
            ...
            {
                foreach (Match mf in linkRegex.Matches(file))
                {
