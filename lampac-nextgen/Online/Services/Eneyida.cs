...
namespace Online.Services
{
    public struct EneyidaInvoke
    {
        ...
        public EneyidaInvoke(string host, string apihost, HttpHydra httpHydra, Func<string, string> onstreamfile)
        {
        ...
        public async Task<EmbedModel> Embed(string original_title, int year, string href, bool similar)
        {
            ...
            await httpHydra.GetSpan(iframeUri, content =>
            {
                ...
                if (Regex.IsMatch(content, "file: ?'\\["))
                {
                    try
                    {
                        var root = JsonSerializer.Deserialize<Models.Tortuga.Voice[]>(Rx.Match(content, "file: ?'([^\n\r]+)',"), new JsonSerializerOptions
                        {
                    ...
                    {
                        Serilog.Log.Error(ex, "{Class} {CatchId}", "Eneyida", "id_vv892psa");
                    }
                }
                else
                {
