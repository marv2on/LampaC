...
namespace Online.Services
{
    public struct VideoCDNInvoke
    {
        ...
        public VideoCDNInvoke(OnlinesSettings init, Func<string, string, Task<string>> onget, Func<string, string> onstreamfile, string host = null)
        {
            this.host = host != null ? $"{host}/" : null;
            this.scheme = init.scheme;
            this.iframeapihost = init.host;
        ...
        public ITplResult Tpl(EmbedModel result, string imdb_id, long kinopoisk_id, string title, string original_title, string t, int s, bool rjson = false)
        {
            ...
            if (result.type is "movie" or "anime")
            {
                ...
                foreach (var voice in result.movie)
                {
                    ...
                    if (first != null)
                        mtpl.Append(name, first.link, streamquality: streamquality);
                }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        foreach (var episode in season.folder)
                        {
                            ...
                            var first = streamquality.Firts();
                            if (first != null)
                                etpl.Append($"{e} серия", title ?? original_title, s.ToString(), e, first.link, streamquality: streamquality);
                        }
