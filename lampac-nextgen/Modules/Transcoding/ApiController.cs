...
namespace Transcoding.Controllers
{
    ...
    public sealed class ApiController : Controller
    {
        ...
        public ActionResult TranscodingJs(string token)
        {
            ...
            var plugin = FileCache.ReadAllText($"{ModInit.modpath}/plugin.js", "transcoding.js")
        ...
        public async Task<IActionResult> StartM3u8(string src, int a, int s, bool subtitles, bool live)
        {
            ...
            if (src.Contains("/proxy/") && src.Contains(".mkv"))
            {
                ...
                src = ProxyLink.Decrypt(hash.AsSpan(), null)?.uri;
                if (string.IsNullOrWhiteSpace(src))
                    return BadRequest(new { error = "src decrypt" });
            }

            var defaults = ModInit.conf;
            ...
            string uri = $"{CoreInit.Host(HttpContext)}/transcoding/{job.StreamId}/{(live ? "live" : "main")}.m3u8";
        ...
        public async Task<IActionResult> Start([FromBody] TranscodingStartRequest request)
        {
            ...
            {
                job.StreamId,
                playlistUrl = AccsDbInvk.Args($"{CoreInit.Host(HttpContext)}/transcoding/{job.StreamId}/{(job.Context.live ? "live" : "main")}.m3u8", HttpContext),
                subtitlesUrl = job.Context.subtitles ? AccsDbInvk.Args($"{CoreInit.Host(HttpContext)}/transcoding/{job.StreamId}/subtitles", HttpContext) : null,
        ...
        public async Task<IActionResult> Segment(string streamId, string file)
        {
            ...
            if (job.Context.live == false && resolved == null && !file.Contains(".vtt"))
            {
                ...
                do
                {
                    ...
                    {
                        Log.Error(ex, "CatchId={CatchId}", "id_om7pdvok");
                    }
                }
                while (sw.Elapsed < fileExistsTimeout);
            }
            ...
            while (segmentIndex.HasValue && sw.Elapsed < fileExistsTimeout)
            {
                ...
                {
                    Log.Error(ex, "CatchId={CatchId}", "id_4pxipm5i");
                }
            ...
            if (job.Context.subtitles && job.Context.ffprobe.ContainsKey("streams"))
            {
                foreach (var s in job.Context.ffprobe["streams"])
                {
                    ...
                    if (s.Value<string>("codec_name") is "subrip" or "webvtt" or "ass" or "ssa" or "mov_text" or "ttml" or "sami")
                    {
                        ...
                        string uri = $"{CoreInit.Host(HttpContext)}/transcoding/{streamId}/{AccsDbInvk.Args($"subs_{subIndex}.vtt", HttpContext)}";
        ...
        public IActionResult Status(string streamId)
        {
            ...
            {
                Log.Error(ex, "CatchId={CatchId}", "id_iuxl2cac");
            }
            ...
            var endpoints = new object[]
            {
                ...
                new {
                    ...
                    body = new {
                        ...
                        audio = ModInit.conf.audioOptions,
                        hls = ModInit.conf.hlsOptions
                    },
