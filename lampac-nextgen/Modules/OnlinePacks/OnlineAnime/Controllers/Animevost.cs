...
namespace OnlineAnime.Controllers
{
    ...
    {
        public Animevost() : base(ModInit.conf.Animevost) { }
        ...
        async public Task<ActionResult> Index(string title, int year, string uri, int s, bool rjson = false, bool similar = false)
        {
            ...
            {
                #region Поиск
                var cache = await InvokeCacheResult<List<(string title, string year, string uri, string s, string img)>>($"animevost:search:{title}:{similar}", TimeSpan.FromHours(4), async e =>
                {
            ...
            {
                #region Серии
                var cache = await InvokeCacheResult<List<(string episode, string id)>>($"animevost:playlist:{uri}", TimeSpan.FromHours(1), async e =>
                {
        ...
        rhubFallback:
            var cache = await InvokeCacheResult<List<(string l, string q)>>($"animevost:video:{id}", 20, async e =>
            {
                ...
                await httpHydra.GetSpan($"{init.host}/frame5.php?play={id}&old=1", iframe =>
                {
