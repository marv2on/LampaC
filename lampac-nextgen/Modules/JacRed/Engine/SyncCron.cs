            ...
            while (true)
            {
                try
                {
                    if (ModInit.conf.typesearch == "red" && !string.IsNullOrWhiteSpace(ModInit.conf.Red.syncapi))
                    {
                        ...
                        var root = await Http.Get<RootObject>($"{ModInit.conf.Red.syncapi}/sync/fdb/torrents?time={lastsync}", timeoutSeconds: 300, MaxResponseContentBufferSize: 100_000_000);
                ...
                catch
                {
                    ...
                    {
                        Serilog.Log.Error(ex, "{Class} {CatchId}", "SyncCron", "id_ri1xl9r3");
                    }
                }
