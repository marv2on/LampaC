...
namespace JacRed.Controllers
{
    ...
    {
        static readonly Serilog.ILogger Log = Serilog.Log.ForContext<LostfilmController>();
        ...
        {
            if (!jackett.Lostfilm.enable || ModInit.conf.disableJackett)
        ...
        async Task<byte[]> getTorrent(string episodeid)
        {
            ...
            {
                Log.Error(ex, "CatchId={CatchId}", "id_w5bfkqrz");
            }
        ...
        async static ValueTask<string> getCookie()
        {
            ...
            {
                Log.Error(ex, "CatchId={CatchId}", "id_fh8tg4sn");
            }
