...
namespace NextHUB.Controllers
{
    public static class Root
    {
        ...
        public static NxtSettings goInit(string plugin)
        {
            ...
            if (ModInit.conf.NextHUB_sites_enabled != null && !ModInit.conf.NextHUB_sites_enabled.Contains(plugin))
                return null;

            if (!File.Exists($"{ModInit.modpath}/NextHUB/sites/{plugin}.yaml"))
                return null;
            ...
            if (!memoryCache.TryGetValue(memKey, out NxtSettings init))
            {
                try
                {
                    ...
                    string yaml = File.ReadAllText($"{ModInit.modpath}/NextHUB/sites/{plugin}.yaml");
                    var target = deserializer.Deserialize<Dictionary<object, object>>(yaml);

                    foreach (string y in new string[] { "_", plugin })
                    {
                        if (File.Exists($"{ModInit.modpath}/NextHUB/override/{y}.yaml"))
                        {
                            // Чтение пользовательского YAML-файла
                            string myYaml = File.ReadAllText($"{ModInit.modpath}/NextHUB/override/{y}.yaml");
        ...
        static string changeFileId(string plugin, IMemoryCache memoryCache)
        {
            ...
            if (!memoryCache.TryGetValue(memKey, out string fileKeyId))
            {
                ...
                sb.Append(File.GetLastWriteTimeUtc($"{ModInit.modpath}/NextHUB/sites/{plugin}.yaml").ToString());

                foreach (string y in new string[] { "_", plugin })
                {
                    if (File.Exists($"{ModInit.modpath}/NextHUB/override/{y}.yaml"))
                        sb.Append(File.GetLastWriteTimeUtc($"{ModInit.modpath}/NextHUB/override/{y}.yaml").ToString());
                }
