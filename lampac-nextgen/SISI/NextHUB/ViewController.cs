...
namespace NextHUB.Controllers
{
    public class ViewController : BaseSisiController<NxtSettings>
    {
        ...
        {
            string infile = $"{ModInit.modpath}/NextHUB/sites/{evalcode}";
            ...
            if (evalcode.Contains("{include:"))
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    if (System.IO.File.Exists($"{ModInit.modpath}/NextHUB/utils/{file}"))
                    {
                        string includeCode = FileCache.ReadAllText($"{ModInit.modpath}/NextHUB/utils/{file}");
