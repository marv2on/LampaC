(function () {
  ...
  var templates = {
    ...
    activity: html$X,
    settings: html$V,
    settings_main: html$U,
    settings_interface: html$T,
    settings_player: html$S,
    settings_more: html$R,
    settings_plugins: html$Q,
    settings_cloud: html$P,
    settings_account: html$O,
    settings_server: html$N,
    scroll: html$W,
    items_line: html$M,
    card: html$L,
    player: html$K,
    ...
    player_info: html$H,
    selectbox: html$G,
    selectbox_item: html$F,
    info: html$E,
    more: html$C,
    search: html$B,
    settings_input: html$A,
    modal: html$z,
    modal_loading: html$y,
    modal_pending: html$x,
    empty: html$w,
    about: html$v,
    error: html$u,
    filter: html$D,
    ...
    torrent: html$k,
    torrent_file: html$j,
    torrent_noconnect: html$i,
    torrent_file_serial: html$f,
    torrent_nocheck: html$h,
    torrent_nohash: html$g,
    account_add_device: html$e
  };
  ...
  var html$a, active$3, scroll$2, last$2;
  ...
  function open$1(params) {
    ...
    html$a.find('.modal__body').append(scroll$2.render());
    bind$2(params.html);
  ...
  var api = Utils.protocol() + 'cub.red/api/';
  var notice_load = {
    time: 0,
    data: []
  };
  ...
  function save$3(method, type, card) {
    ...
    if (account.token && Storage.field('account_use')) {
      ...
      network$7.clear();
      network$7.silent(api + 'bookmarks/' + method, update$5, false, {
        type: type,
      ...
      html.find('.selector').on('hover:enter', function () {
        ...
        Input.edit({
          ...
          if (new_value && new_value.length == 6 && !isNaN(code)) {
            network$7.clear();
            network$7.silent(api + 'device/add', function (result) {
  ...
  var baseurl$1 = Utils.protocol() + 'apitmdb.cub.red/3/';
  ...
  function full$2() {
    ...
    get$4(params.method + '/' + params.id, params, function (json) {
      json.source = 'tmdb';
      if(json.id) status.append('movie', json);
	  else status.error()
    }, status.error.bind(status));
  }
  ...
  var baseurl = Utils.protocol() + 'tmdb.cub.red/';
  ...
  function menuCategory$1(params, oncomplite) {
    ...
    menu.push({
      title: 'Сейчас смотрят',
      url: '?cat=' + params.action + '&sort=now_playing' + args
    });

    if (params.action == 'tv') {
      menu.push({
        title: 'Новые серии',
        url: '?cat=' + params.action + '&sort=update' + args
      });
    }

    menu.push({
      title: 'Популярное',
      url: '?cat=' + params.action + '&sort=top' + args
    });
    menu.push({
      title: 'Последнее добавление',
      url: '?cat=' + params.action + '&sort=latest' + args
    });
    menu.push({
      title: 'Новинки этого года',
      url: '?cat=' + params.action + '&sort=now' + args
    });
    menu.push({
      title: 'С высоким рейтингом',
      url: '?cat=' + params.action + '&sort=latest&vote=7' + args
    });
    oncomplite(menu);
  }
  ...
  function relise() {
    ...
    var oncomplite = arguments.length > 1 ? arguments[1] : undefined;
    var onerror = arguments.length > 2 ? arguments[2] : undefined;
    network$4.silent(Utils.protocol() + 'tmdb.cub.red?sort=releases&results=25&page=' + params.page, function (json) {
  ...
  function create$k(data) {
    ...
    if (vote > 0) card.append('<div class="card__vote">' + vote.toFixed(2) + '</div>');
  ...
  var elems$1 = {
    ...
    timeline: $('.player-panel__timeline', html$9),
    quality: $('.player-panel__quality', html$9)
  };
  /**
  ...
  html$9.find('.player-panel__tstart').on('hover:enter', function (e) {
    listener$9.send('to_start', {});
  });
  html$9.find('.player-panel__tend').on('hover:enter', function (e) {
  ...
  html$9.find('.player-panel__line:eq(1) .selector').attr('data-controller', 'player_panel');
  ...
  var html$8 = Template.get('player_video');
  var display = html$8.find('.player-video__display');
  var paused = html$8.find('.player-video__paused');
  var subtitles$1 = html$8.find('.player-video__subtitles');
  var timer$3 = {};
  ...
  function loader(status) {
    wait = status;
    html$8.toggleClass('video--load', status);
  }
  /**
  ...
  var html$7 = Template.get('player_info');
  var listener$7 = start$3();
  var network$3 = new create$n();
  var elems = {
    name: $('.player-info__name', html$7),
    size: $('.value--size span', html$7),
    stat: $('.value--stat span', html$7),
    speed: $('.value--speed span', html$7),
    error: $('.player-info__error', html$7)
  };
  ...
  function stat$1(url) {
    ...
    var update = function update() {
      ...
      if (!html$7.hasClass('info--visible')) {
        wait++;
  ...
  function toggle$4(status) {
    html$7.toggleClass('info--visible', status);
  }
  /**
   * Уничтожить
   */
  ...
  var chrome = false;
  var img;
  var html$6 = Template.get('screensaver');
  var movies = [];
  var timer$1 = {};
  var position = 0;
  var slides$1 = 'one';
  var direct = ['lt', 'rt', 'br', 'lb', 'ct'];
  html$6.on('click', function () {
  ...
  function nextSlide() {
    ...
    img.onload = function () {
      var to = $('.screensaver__slides-' + (slides$1 == 'one' ? 'two' : 'one'), html$6);
      to[0].src = img.src;
      to.removeClass(direct.join(' ') + ' animate').addClass(direct[Math.floor(Math.random() * direct.length)]);
      setTimeout(function () {
        $('.screensaver__title', html$6).removeClass('visible');
        $('.screensaver__slides-' + slides$1, html$6).removeClass('visible');
        slides$1 = slides$1 == 'one' ? 'two' : 'one';
        to.addClass('visible').addClass('animate');

        if (movie) {
          setTimeout(function () {
            $('.screensaver__title-name', html$6).text(movie.title || movie.name);
            $('.screensaver__title-tagline', html$6).text(movie.original_title || movie.original_name);
            $('.screensaver__title', html$6).addClass('visible');
          }, 500);
        }
      }, 3000);
    };
  ...
  function stopSlideshow() {
    ...
    html$6.fadeOut(300, function () {
      html$6.removeClass('visible');
    });
  ...
  var html$5 = Template.get('player');
  html$5.append(Video.render());
  html$5.append(Panel.render());
  html$5.append(Info.render());
  var callback$2;
  var work = false;
  ...
  function play(data) {
    ...
    if (Platform.is('webos') && Storage.field('player') == 'webos') {
      ...
      preload(data, function () {
        ...
        Info.set('name', data.title);
        if (!preloader.call) $('body').append(html$5);
        toggle$1();
        Panel.show(true);
      });
    }
  ...
  function create$b(data) {
    ...
    this.plugins = function () {
      ...
      events.forEach(function (a) {
        var plugin = $('<div class="videos__tab selector">' + (a.name || 'Без названия').split('~')[0].split('-')[0].trim() + '</div>');
    ...
    this.call = function (target, json) {
      ...
      if (json.method == 'play') {
        ...
        network.silent(metaData(json.url), function (result) {
          close();
    ...
    this.get = function (link, push_history) {
      ...
      network.clear();
      network.silent(metaData(link), function (result) {
        if (result.trim()) {
          if (api.split('.').pop() == 'js') {
            try {
              js = eval('(' + result + ')');
  ...
  function ready() {
    html$3.find('.selector').on('hover:enter', function (e) {
      ...
      if (action == 'movie' || action == 'tv' || action == 'anime' || action == 'multfilm') {
        ...
        Api.menuCategory({
          ...
          Select.show({
            ...
            onSelect: function onSelect(a) {
              Activity$1.push({
                url: a.url,
                title: (action == 'movie' ? 'Фильмы' : action == 'anime' ? 'Аниме' : action == 'multfilm' ? 'Мульты' : 'Сериалы') + ' - ' + source.toUpperCase(),
                component: 'category_full',
                source: source,
                page: 1
              });
  ...
  function catalog() {
    Api.menu({
      ...
      Select.show({
        ...
        onSelect: function onSelect(a) {
          Activity$1.push({
            url: Storage.field('source') == 'cub' ? '?sort=top' : 'movie/popular',
  ...
  function connect() {
    ...
    try {
      socket = new WebSocket('wss://cub.red:8010');
    } catch (e) {
  ...
  function startApp() {
    ...
    Favorite.listener.follow('add,added', function (e) {
      if (e.where == 'history' && e.card.id) {
        $.get(Utils.protocol() + 'tmdb.cub.red/watch?id=' + e.card.id + '&cat=' + (e.card.original_name ? 'tv' : 'movie'));
      }
    });
    Lampa.Listener.send('app', {
      type: 'ready'
