(function () {
    ...
    function Sisi(object) {
      ...
      var unic_id = Lampa.Storage.get('device_uid', '');
      var filter = new Lampa.Filter(object);
      var filter_sources = [];

      function account(url) {
        if (url.indexOf(window.plugin_sisi_localhost.replace(/http:\/\//g, '').split('/')[0]) >= 0) {
		...
		if (url.indexOf('uid=') == -1) 
          Lampa.Utils.addUrlComponent(url, 'uid=' + unic_id);
        return url;
      }
      ...
      this.create = function () {
		...
		var uid = $('<div style="-webkit-align-self: center; -ms-flex-item-align: center; align-self: center; font-size: 1.2em;"></div>');
      ...
      this.append = function (data) {
        ...
        data.forEach(function (element) {
          ...
          card.on('hover:enter', function () {
            if (element.json) {
              if (!wait_parse_video) {
                network["native"](account(element.video + '&json=true'), function (resqualitys) {
                  wait_parse_video = false;

                  if (!resqualitys.qualitys)
				  {
					  var video = {
						title: element.name,
						url: resqualitys[Lampa.Arrays.getKeys(resqualitys)[0]],
						quality: resqualitys
					  };
					  
					  if (video.url.includes('ptx/strem') || video.url.includes('tizam/vidosik'))
						video.url = account(video.url);
				  ...
				  else 
				  {
					  var video = {
						title: element.name,
						url: resqualitys.qualitys[Lampa.Arrays.getKeys(resqualitys.qualitys)[0]],
						quality: resqualitys.qualitys
					  };
    ...
    function startPlugin() {
      ...
      window.plugin_sisi_localhost = '{localhost}/sisi';
